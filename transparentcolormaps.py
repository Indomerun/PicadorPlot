...
def make_alphablended_cmap(cmapName, new_cmapName):
    ...
    new_cmap = mpl.colors.LinearSegmentedColormap.from_list(new_cmapName, alphaBlend(src_RGBA, dst_RGBA))
    mpl.cm.register_cmap(new_cmapName, new_cmap)


def getColorList(colorString):
    i = 0
    colorList = []
    block = False
    while i < len(colorString):
        if block is False:
            if colorString[i] == '{':
                block = True
                i += 1
                colorList.append(colorString[i])
                i += 1
            elif colorString[i] == '}':
                return None
            else:
            ...
            if colorString[i] == '{':
                ...
                colorList[-1] += colorString[i]
                i += 1
    return colorList


def make_linear_colormap(name, string):
    """Return a LinearSegmentedColormap
    seq: a sequence of RGB-tuples.
    """
    seq = getColorList(string)
    nElements = len(seq)
    colors = []
    for i in range(nElements):
        if seq[i] == 't':
            ...
            colors.append((1, seq[i], seq[i]))
    cdict = {'alpha': [], 'red': [], 'green': [], 'blue': []}
    i = 0
    for element in colors:
        position = float(i)/(nElements-1)  # float casting in case of Python2.7 usage
        r1, g1, b1 = mpl.colors.colorConverter.to_rgb(element[1])
        r2, g2, b2 = mpl.colors.colorConverter.to_rgb(element[2])
        cdict['alpha'].append([position, element[0], element[0]])
        cdict['red'].append([position, r1, r2])
        cdict['green'].append([position, g1, g2])
        cdict['blue'].append([position, b1, b2])
        i += 1
    cmap = mpl.colors.LinearSegmentedColormap('CustomMap', cdict)
    mpl.cm.register_cmap(name, cmap)
...
def physicalBlend(src_RGBA, dst_RGBA):
    src_A = src_RGBA[..., -1]
    src_R = src_RGBA[..., 0]
    src_G = src_RGBA[..., 1]
    src_B = src_RGBA[..., 2]
    ...
    out_A = np.ones(dst_A.shape)
    red_A = (1-src_R) + (1-dst_R) * (1 - (1-src_R))
    out_R = div0((src_R * (1-src_R) + dst_R * (1-dst_R) * (1 - (1-src_R))) , red_A)
    green_A = (1-src_G) + (1-dst_G) * (1 - (1-src_G))
    out_G = div0((src_G * (1-src_G) + dst_G * (1-dst_G) * (1 - (1-src_G))) , green_A)
    blue_A = (1-src_B) + (1-dst_B) * (1 - (1-src_B))
    out_B = div0((src_B * (1-src_B) + dst_B * (1-dst_B) * (1 - (1-src_B))) , blue_A)
